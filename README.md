# Adidas-Sales-Analysis-Excel

<p align="center">
  <img src="https://github.com/anthonymike180/Adidas-Sales-Analysis-Excel/blob/main/Image.png" width="700">
</p>

This is a Microsoft Excel based sales analysis and dashboard project for sales data from an Adidas store/brand.  
It’s designed to help analyze sales performance, product performance, and generate insights from raw sales data using Excel.

## Project Overview

This project transforms raw sales data into actionable insights and reports using Excel.  
It includes analyses such as:

- Overall sales performance over time  
- Product-level performance  
- Monthly / quarterly sales trends  
- Customer or sales category breakdowns (if data supports it)  
- Visual dashboards and charts for quick insights  

## Project Files

- <a href="https://github.com/anthonymike180/Adidas-Sales-Analysis-Excel/blob/main/Addidas_Sales_Analysis.xlsx">Excel file / Workings</a>
- <a href="https://github.com/anthonymike180/Adidas-Sales-Analysis-Excel/blob/main/Screenshots.pdf">Dashboard Screenshots</a>

## How to Use / View the Dashboard

1. Download or clone the repository.  
2. Open <a href="https://github.com/anthonymike180/Adidas-Sales-Analysis-Excel/blob/main/Addidas_Sales_Analysis.xlsx">Excel file / Workings</a> in Microsoft Excel.  
3. Navigate through the sheets as you wish.  
4. Explore charts and pivot-based tables to view insights, trends, and performance metrics.

## Tools & Techniques Used

- Microsoft Excel  
  - Data cleaning & structuring  
  - Pivot tables & pivot charts  
  - Formulas and calculations  
  - Visual charts / graphs  
  - Formatting for dashboard-style presentation
  
## Objective

This project is a demonstration of Excel-based data analysis and dashboard building.  
- Analyze Adidas sales data and identify key performance trends.
- Build an interactive Excel dashboard for easy visualization of sales insights.
- Track sales performance across products, regions, and time periods.
- Transform raw data into actionable business insights for decision-making.

## Author
Anthony Michael
anthonymike180@gmail.com

---



